...
 
Commands must be processed asynchronously and the application must be thread-safe. Invalid commands must not stop or block the application You can choose any persistence solution. 
 
Application functionality has to be supported by at leat one unit test. Optionally simulate concurrent user work with another test.
 
